...
class PolkadotGraphVisualization {
    constructor(containerSelector, options = {}) {
        ...
        this.animations = {
            ...
            animationId: 0  // Counter for unique animation IDs
        };
    ...
    updateSimulationParameters() {
        // Store class reference for force functions to avoid context issues
        const self = this;
        ...
        const avgNodeSize = this.state.filteredData.nodes.reduce((sum, n) => { 
            try { const radius = this.getNodeRadius(n); return sum + (isNaN(radius) ? 15 : radius); } catch(e) { return sum + 15; }}, 0) / nodeCount || 15;
        // Initialize collision force if not exists
        ...
        collisionForce
            .radius(function(d) { try { const radius = self.getNodeRadius(d); return (isNaN(radius) ? 15 : radius) + avgNodeSize * 2 + 60; } catch(e) { return 15 + avgNodeSize * 2 + 60; } })  // Massive spacing buffer
    ...
    /**
     * Clear all active animations to prevent context conflicts
     */
    clearAnimations() {
        // TEMPORARILY SIMPLIFIED: Just clear the sets without complex interruption
        if (this.animations) {
            this.animations.activeTransitions.clear();
            this.animations.pulsingNodes.clear();
            this.animations.animationId++;
        }
    }
    
    /**
     * Track an active animation transition
     */
    trackAnimation(transition, nodeId = null) {
        try {
            ...
            this.animations.activeTransitions.add(transition);
            
            if (nodeId) {
                this.animations.pulsingNodes.add(nodeId);
            }
            ...
            if (transition.on) {
                transition.on('end.cleanup', () => {
                    try {
                        this.animations.activeTransitions.delete(transition);
                        if (nodeId) {
            ...
            console.error('Error in trackAnimation:', error);
            return transition; // Return transition even if tracking failed
        }
    }
    ...
    renderNodes() {
        try {
            // Clear any existing animations to prevent context conflicts
            this.clearAnimations();
        ...
        // TEMPORARILY DISABLED: Node removal transitions to isolate context error
        nodeSelection.exit()
            // .transition()
            // .duration(300)
            // .attr('opacity', 0)
        ...
        // Add node circles
        // Store class reference for use in regular functions to avoid D3 context issues
        const self = this;
        
        nodeEnter.append('circle')
            .attr('r', function(d) { return self.getNodeRadius(d); })
            .attr('fill', function(d) { return self.getNodeColor(d); })
            .attr('stroke', function(d) { return self.getNodeStrokeColor(d); })
            .attr('stroke-width', function(d) { return self.getNodeStrokeWidth(d); })
            .attr('opacity', function(d) { return self.getNodeOpacity(d); });
        
        // Add node icons for special types
        nodeEnter.filter(function(d) { return self.shouldShowNodeIcon(d); })
            .append('text')
            .attr('class', 'node-icon')
            .attr('text-anchor', 'middle')
            .attr('dy', '0.35em')
            .attr('font-family', 'Arial Unicode MS')
            .attr('font-size', function(d) { return self.getNodeRadius(d) * 0.8; })
            .attr('fill', 'white')
            .text(function(d) { return self.getNodeIcon(d); });
        
        // Merge enter and update selections
        const nodeUpdate = nodeEnter.merge(nodeSelection);
        
        // TEMPORARILY DISABLED: Node update transitions to isolate context error
        ...
        // Pulsing animations re-enabled - context error has been resolved
            ...
            console.error('Error in renderNodes:', error);
            // Attempt to recover by clearing state and retrying with minimal data
            this.clearAnimations();
    ...
    renderEdges() {
        ...
        // TEMPORARILY DISABLED: Edge removal transitions to isolate context error
        linkSelection.exit()
            // .transition()
            // .duration(300)
            // .attr('opacity', 0)
        ...
        linkEnter.append('line')
            ...
            .attr('stroke', '#FF0000')
            .attr('stroke-width', function(d) { return self.getEdgeHighlightWidth(d); })
            .attr('stroke-opacity', d => d._aboveThreshold ? 0.8 : 0);
        
        // Add main edge line (rendered second, on top)
        linkEnter.append('line')
            .attr('class', 'edge')
            .attr('stroke', function(d) { return self.getEdgeColor(d); })
            .attr('stroke-width', function(d) { return self.getEdgeWidth(d); })
            .attr('stroke-opacity', function(d) { return self.getEdgeOpacity(d); })
            .attr('marker-end', function(d) { return self.getEdgeMarker(d); }); // Arrows re-enabled for direction indication
        
        // Add stroke dasharray for special edge types on the main edge line
        linkEnter.select('.edge')
            .attr('stroke-dasharray', function(d) { return self.getEdgeDashArray(d); });
        
        // Merge enter and update selections
        const linkUpdate = linkEnter.merge(linkSelection);
        
        // TEMPORARILY DISABLED: Edge update transitions to isolate context error
    ...
    renderEdgeLabels() {
        ...
        edgeLabelEnter.append('text')
            ...
            .text(function(d) { return self.getEdgeLabelText(d); });
    ...
    handleZoom(event) {
        try {
            ...
            // Temporarily pause animations during zoom to prevent context conflicts
            const wasAnimating = this.animations.activeTransitions.size > 0;
            if (wasAnimating) {
                // Briefly pause active transitions during zoom
                this.animations.activeTransitions.forEach(transition => {
                    if (transition && typeof transition.interrupt === 'function') {
                        try {
                            transition.interrupt();
                        } catch (interruptError) {
                            // Some transitions might not be interruptible
                        }
                    }
                });
            }
            ...
            if (this.callbacks && this.callbacks.onViewportChange) {
                this.callbacks.onViewportChange(transform, this.state.zoomLevel);
            }
        } catch (error) {
            console.error('Error in handleZoom:', error);
            // Don't rethrow to prevent zoom from breaking completely
        }
    }
     ...
     */
    createDragBehavior() {
        const self = this; // Store class instance reference to avoid context issues
        
        return d3.drag()
            .on('start', function(event, d) {
                // 'this' is the DOM element, 'self' is the class instance
                try {
                    // Only restart simulation if not paused and dragging actively
                    if (!event.active && !self.state.simulationPaused) {
            ...
            .on('end', function(event, d) {
                try {
                    ...
                    if (shouldLock) {
                        ...
                        self.state.lockedNodes.add(d.address);
                        // Keep fixed position
                    ...
                    self.updateNodeLockVisualization();
                } catch (error) {
    ...
    getNodeStrokeColor(nodeData) {
        ...
        // Validate address property exists
        if (!nodeData.address) {
            console.warn('getNodeStrokeColor called with nodeData missing address property:', nodeData);
            return '#ffffff';
        }
        ...
        // Validate state objects exist before checking membership
        if (this.state && this.state.selectedNodes && this.state.selectedNodes.has(nodeData.address)) {
            return this.config.colors.high;
        }
        
        if (this.state && this.state.lockedNodes && this.state.lockedNodes.has(nodeData.address)) {
            return '#FFD700'; // Gold color for locked nodes
        }
        
        if (this.state && this.state.highlightedNodes && this.state.highlightedNodes.has(nodeData.address)) {
    ...
    getNodeStrokeWidth(nodeData) {
        // ULTRA-DEFENSIVE: Check if 'this' is actually the class instance
        if (!this.config || typeof this.config !== 'object') {
            console.error('getNodeStrokeWidth called with wrong context - this is not the class instance:', this);
            return 2; // fallback value
        }
        
        // Comprehensive data validation with error logging
        if (!nodeData) {
            console.warn('getNodeStrokeWidth called with undefined nodeData');
            return this.config.nodes.strokeWidth;
        }
        
        // Validate that nodeData has required properties
        if (typeof nodeData !== 'object') {
            console.warn('getNodeStrokeWidth called with invalid nodeData type:', typeof nodeData);
            return this.config.nodes.strokeWidth;
        }
        
        // Validate address property exists
        if (!nodeData.address) {
            console.warn('getNodeStrokeWidth called with nodeData missing address property:', nodeData);
            return this.config.nodes.strokeWidth;
        }
        ...
        // Validate that state objects exist before checking membership
        if (this.state && this.state.selectedNodes && this.state.selectedNodes.has(nodeData.address)) {
    ...
    getNodeOpacity(nodeData) {
        // Validate nodeData before accessing properties
        if (!nodeData || typeof nodeData !== 'object') {
            console.warn('getNodeOpacity called with invalid nodeData:', nodeData);
            return 0.9;
        }
    ...
    centerOnNode(nodeData) {
        // Skip if zoom is disabled
        if (!this.zoom) {
            console.log("Zoom disabled - centerOnNode not available");
            return;
        }
        try {
            if (!nodeData.x || !nodeData.y) return;
            
            // Clear any conflicting animations before centering
            this.clearAnimations();
    ...
    zoomIn() {
        // Skip if zoom is disabled
        if (!this.zoom) {
            console.log("Zoom disabled - zoomIn not available");
            return;
        }
        try {
            if (this.svg && this.zoom) {
    ...
    zoomOut() {
        // Skip if zoom is disabled
        if (!this.zoom) {
            console.log("Zoom disabled - zoomOut not available");
            return;
        }
        try {
            if (this.svg && this.zoom) {
